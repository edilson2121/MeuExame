"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PublicPagesController = void 0;
const common_1 = require("@nestjs/common");
const public_pages_service_1 = require("./public-pages.service");
let PublicPagesController = class PublicPagesController {
    constructor(pagesService) {
        this.pagesService = pagesService;
    }
    async getPublishedByInstitution(institutionId) {
        return this.pagesService.getPublishedPagesByInstitution(institutionId);
    }
    async getBySlug(slug, institutionId) {
        return this.pagesService.getPublishedPageBySlug(slug, institutionId);
    }
    async getMenu(institutionId) {
        return this.pagesService.getPublicMenu(institutionId);
    }
};
exports.PublicPagesController = PublicPagesController;
__decorate([
    (0, common_1.Get)('institution/:institutionId'),
    __param(0, (0, common_1.Param)('institutionId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], PublicPagesController.prototype, "getPublishedByInstitution", null);
__decorate([
    (0, common_1.Get)('institution/:institutionId/slug/:slug'),
    __param(0, (0, common_1.Param)('slug')),
    __param(1, (0, common_1.Param)('institutionId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], PublicPagesController.prototype, "getBySlug", null);
__decorate([
    (0, common_1.Get)('institution/:institutionId/menu'),
    __param(0, (0, common_1.Param)('institutionId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], PublicPagesController.prototype, "getMenu", null);
exports.PublicPagesController = PublicPagesController = __decorate([
    (0, common_1.Controller)('public/pages'),
    __metadata("design:paramtypes", [public_pages_service_1.PublicPagesService])
], PublicPagesController);
//# sourceMappingURL=public-pages.controller.js.map