# Script para exportar projeto para HTML
Write-Host "====================================" -ForegroundColor Cyan
Write-Host "   EXPORTADOR DE PROJETO             " -ForegroundColor Cyan
Write-Host "====================================" -ForegroundColor Cyan
Write-Host ""

# Pastas a ignorar
$ignorarPastas = @("node_modules", ".next", "dist", "build", ".git", "coverage", ".vscode", ".idea", "tmp", "temp", "logs", "cache")
$ignorarExtensoes = @(".log", ".lock", ".map", ".png", ".jpg", ".jpeg", ".gif", ".ico", ".woff", ".woff2", ".ttf", ".eot", ".mp4", ".mp3", ".webp")
...
<head>
    <meta charset="UTF-8">
    <title>Projeto MeuExame - Codigo Completo</title>
    <style>
...
<body>
    <div class="header">
        <h1>PROJETO MEUEXAME - CODIGO COMPLETO</h1>
...
foreach ($pasta in $pastas) {
    ...
    $html += "            <li>$contador. PASTA: $nomePasta</li>`n"
    $contador++
}
...
"@
    ...
    try {
        ...
        if ($linhas) {
            ...
            $html += "   Arquivo vazio ou binario`n"
        }
    } catch {
        $html += "   Erro ao ler o arquivo`n"
    }
