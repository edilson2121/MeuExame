...
const TransactionMpesa = () => {
  ...
  const startPollingStatus = (transactionId: string) => {
    ...
    const interval = setInterval(async () => {
      attempts++;
      console.log(`🔍 [M-Pesa] Verificando status (tentativa ${attempts}/${maxAttempts})...`);
      ...
      try {
        ...
        if (response.data.status === 'completed') {
          ...
          showModal(
            ...
            "Seu pagamento via M-Pesa foi confirmado! O acesso total ao MeuExame.com foi liberado.",
            transactionId
          );
        } 
          ...
          showModal(
            "cancel",
            "✗ Pagamento Cancelado",
            "O pagamento foi cancelado. Nenhum valor foi cobrado da sua conta M-Pesa.",
            transactionId
          );
        }
        ...
        console.error("❌ [M-Pesa] Erro no polling:", error);
  ...
  const handlePagamento = async () => {
    if (!numeroTelefone || numeroTelefone.length < 9) {
      showModal("error", "Número Inválido", "Por favor, digite um número de telefone válido com 9 dígitos (ex: 84XXXXXXX)");
      return;
    }

    const prefixo = numeroTelefone.substring(0, 2);
    if (prefixo !== "84" && prefixo !== "85") {
      showModal("error", "Número M-Pesa Inválido", "O número deve começar com 84 ou 85.\n\nExemplo: 84 123 4567 ou 85 123 4567");
      return;
    }
    ...
    try {
      ...
      console.log("📱 [M-Pesa] Resposta:", response.data);
      
      if (response.data.confirmed === true) {
        showModal(
          "success",
          "✓ Pagamento Confirmado",
          "Seu pagamento via M-Pesa foi processado com sucesso! O acesso total ao MeuExame.com foi liberado.",
      ...
      console.log("❌ [M-Pesa] Erro:", errorData);
      
      if (isCancelled) {
        showModal(
          "cancel",
          "✗ Operação Cancelada",
          "Você cancelou a operação no seu celular M-Pesa. Nenhum valor foi cobrado da sua conta."
        );
      }
        ...
        showModal(
          "error",
          "💰 Saldo Insuficiente",
          `Seu saldo M-Pesa é insuficiente para completar esta transação de 299 MZN.\n\nRecarregue sua conta e tente novamente.` 
        );
      }
        ...
        showModal(
          "error",
          "📱 Número Inválido",
          "O número informado não é válido ou não está registrado no M-Pesa."
        );
      }
      else {
        showModal(
  ...
  return (
    <div className="flex flex-col items-center px-4 py-6">
      <h1 className="text-xl font-bold mb-4 text-gray-900">Pagar via M-Pesa</h1>

      <div className="w-full max-w-md">
        <label htmlFor="telefone" className="block text-sm font-medium text-gray-700 mb-2">
          EX: 84XXXXXXX ou 85XXXXXXX
        </label>
        <input
          type="tel"
          id="telefone"
        ...
        <p className="text-red-600 mt-2 text-sm">
          * Digite o seu número de telefone M-Pesa e clique em pagar agora!
        </p>
      </div>
...
export default TransactionMpesa;
